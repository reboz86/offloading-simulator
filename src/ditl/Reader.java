...
public class Reader<I> implements Generator {
	...
	public void incr(long incr_time) throws IOException {
		cur_time += incr_time;
		if (D) System.out.println("Reader.incr: "+cur_time);
	...
	private void step() throws IOException {
		if (D) System.out.println("Reader.step");
		stepNextTime();
	...
	public void setBus(Bus<I> bus) {
		if(D)System.out.println("Reader.setBus: "+ bus.toString());
		_bus = bus;
	}
	
	private void peekLine() throws IOException {
		if (D) System.out.println("Reader.peekLine");
	...
	private void stepNextTime() throws IOException {
		if (D) System.out.println("Reader.stepNextTime");
		while ( ! has_next_time ){
			peekLine();
		}
		prev_time = next_time;
	...
	private String getNextItem() throws IOException {
		if (D) System.out.println("Reader.getNextItem");
		if ( ! has_next_item )
			return null;
